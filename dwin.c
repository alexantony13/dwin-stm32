...
uint8_t dwin_init(dwin_t *dwin)
{
  ...
  if (dwin->rx_ring_buffer.size != 0)
  {
    if (dwin->rx_ring_buffer.size < DWIN_RX_CIRC_BUF_MAX_LEN)
    {
      ...
      {
        if (HAL_UARTEx_ReceiveToIdle_DMA(dwin->huart,
            dwin->rx_ring_buffer.buf_ptr, dwin->rx_ring_buffer.size) != HAL_OK)
        {
          ret_status = 4;
        }
      }
...
static uint8_t dwin_ring_buffer_dequeue(dwin_t *dwin, uint8_t *data)
{
  ...
  {
    if (dwin->rx_ring_buffer.tail_index != dwin->rx_ring_buffer.head_index)
    {
...
uint8_t dwin_process(dwin_t *dwin, uint32_t c_tick)
{
  ...
  if (dwin->status == DWIN_STATUS_UART_ERROR)
  {
    dwin->rx_status = DWIN_RX_STATUS_WAITING_HEADER;
    dwin->tx_status = DWIN_TX_STATUS_IDLE;
    HAL_UART_Abort(dwin->huart);
    dwin->rx_ring_buffer.head_index = -1;
    dwin->rx_ring_buffer.tail_index = -1;
    HAL_UARTEx_ReceiveToIdle_DMA(dwin->huart, dwin->rx_ring_buffer.buf_ptr,
        dwin->rx_ring_buffer.size);
    dwin->status = DWIN_STATUS_OK;
  }
    ...
    if (dwin_ring_buffer_dequeue(dwin, &rx_data) == 0)
    {
      ...
      switch (dwin->rx_status)
      {
        ...
        case DWIN_RX_STATUS_WAITING_FN_CODE:
          ...
          dwin->rx_data_byte_len = dwin->rx_frame_buffer[DWIN_FRAME_LEN] - 1;
  ...
  if (dwin->rx_status == DWIN_RX_STATUS_DATA_RECEIVED)
  {
    ...
    {
      uint16_t address = (dwin->rx_frame_buffer[DWIN_FRAME_DATA_START] << 8)
          | (dwin->rx_frame_buffer[DWIN_FRAME_DATA_START + 1]);

      uint8_t data_count = dwin->rx_frame_buffer[DWIN_FRAME_DATA_START + 2];
      if (data_count == 1)
      {
        uint16_t data = (dwin->rx_frame_buffer[DWIN_FRAME_DATA_START + 3] << 8)
            | (dwin->rx_frame_buffer[DWIN_FRAME_DATA_START + 4]);
        for (uint8_t i = 0; i < DWIN_CALLBACK_ADDR_MAX_COUNT; ++i)
        {
      ...
      if (dwin->tx_status == DWIN_TX_STATUS_ACK_WAITING)
      {
        if (dwin->rx_frame_buffer[DWIN_FRAME_FUNC_CODE] == 0x82)
        {
          if ((dwin->rx_frame_buffer[DWIN_FRAME_DATA_START] == 0x4f)
              && (dwin->rx_frame_buffer[DWIN_FRAME_DATA_START + 1] == 0x4b))
          {
  ...
  {
    if ((c_tick - dwin->rx_frame_start_tick) >= dwin->rx_frame_timeout_ticks)
    {
      dwin->rx_status = DWIN_RX_STATUS_WAITING_HEADER;
    }
  }
    ...
    uint16_t vp_data_len)
{
  ...
  else
  {
    ...
    else
    {
      ...
      dwin->tx_frame_buffer[DWIN_FRAME_DATA_START + 1] = vp_start_addr & 0x00ff;

      for (uint8_t i = 0; i < vp_data_len; ++i)
      {
        dwin->tx_frame_buffer[DWIN_FRAME_DATA_START + 2 + (2 * i)] = vp_data[i]
            >> 8;
    ...
    if (HAL_OK
        == HAL_UART_Transmit_DMA(dwin->huart, dwin->tx_frame_buffer,
            tx_frame_len))
    {
      dwin->tx_last_sent_tick = HAL_GetTick();
